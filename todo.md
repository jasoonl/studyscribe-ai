...
- [x] Fix Vercel authentication reporting existing accounts as not found while preserving invite-gated signup and preventing sign-in from creating new accounts. Added case- and whitespace-insensitive email lookup across password login, Google existing-user matching, invite validation, and registration normalization; production health, session guard, and Google OAuth checks pass.
- [x] Migrate the five existing Manus user accounts into project-owned TiDB, preserve password hashes and Google IDs without exposing them, and validate Vercel login against the migrated records while keeping invite-gated signup unchanged. The five source and destination records match exactly by non-reversible email fingerprints, IDs, roles, verification flags, and password/Google-ID presence; Vercel now reads TiDB while Manus remains the rollback source.
- [x] Diagnose and fix migrated-account email/password verification still returning invalid credentials on Vercel, without exposing password hashes or weakening invite-gated signup. The failure was an application lookup-module import defect, not a password-hash mismatch; the repair preserves invite-gated signup and uses no credential logging.
- [x] Distinguish Google-only migrated accounts from wrong passwords in email login, and direct passwordless users to Google sign-in or password reset without exposing account enumeration details. Safe diagnostics classify missing users, passwordless accounts, and mismatched passwords without logging identifiers or secrets.
- [x] Compare Manus and Vercel authentication identity/session behavior because both existing email/password and Google accounts work on Manus but fail on Vercel. Vercel uses the migrated TiDB records and the Google callback uses the registered Vercel origin; the remaining email failure was the missing Drizzle import.
- [x] Add temporary non-sensitive authentication rejection diagnostics that log only the failure category and hash metadata, never email addresses, passwords, or hashes, then remove the diagnostics after identifying the Vercel failure. Diagnostics identified the live configuration issue and were removed before this repair.
...
- [x] Inspect the latest Vercel authentication retry logs and fix the remaining Google callback and email/password failures based on their exact non-sensitive categories. The email category was `user_not_found`; the live deployment had incomplete provider fields masking a valid DATABASE_URL, and the repaired precedence now falls back correctly.
- [x] Inspect the fresh Vercel email-login failure category from the latest retry and apply the exact verified authentication repair. Production logs showed `user_not_found`; local reproduction exposed that `getUserByEmail` referenced Drizzle `eq`/`sql` without the required import in the deployed lookup module. The import is repaired and focused auth tests, TypeScript, and production build pass.
- [ ] Verify the production alias serving the latest authentication repair and fix the remaining email/password login failure from the actual live route, then remove temporary diagnostics.
- [x] Fix database configuration precedence so incomplete `TIDB_*` or `DB_*` variables cannot mask a valid `DATABASE_URL`; then redeploy and verify the migrated account lookup on Vercel. Complete field sets still take precedence, while incomplete sets now safely fall back to DATABASE_URL; regression coverage passes.
